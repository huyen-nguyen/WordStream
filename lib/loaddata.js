...
function loadBlogPostDataInit(draw, space, timeArcs) {
    d3.json("data/huff2012.json", function (error, inputData) {
        if (error) {
            console.log(error);
        }
        else {
            var temp = JSON.parse(JSON.stringify(inputData));
            var topData = getTop(temp, 20);        // same input, output specific
            data = tfidf(topData);
            gdata = data;
            draw(data);
            // timeArcs()
...
function loadBlogPostData(draw, space, timeArcs){
    if (fileName === "data/"+"Huffington"+".tsv"){
        d3.json("data/huff2012.json", function (error, inputData) {
            if (error) {
                console.log(error);
            }
    ...
    d3.tsv(fileName, function(error, rawData) {
        ...
        rawData = rawData.filter(function(d){
            ...
            var startDate =  inputFormat.parse('2012-1-01T00:00:00');
        ...
        data = d3.keys(data).map(function(date, i){
            ...
            topics.forEach(topic => {
                ...
                words[topic] = d3.keys(counts).map(function(d){
                 ...
                 words[topic] = words[topic].slice(0, Math.min(words[topic].length, 500));
        ...
        for (var i = 0; i < data.length; i ++){
            for (var j in sub){
                ...
                data[i].words[sub[j]]=data[i].words[sub[j]].slice(0, Math.min(data[i].words[sub[j]].length, 30 ));
            }
        }
