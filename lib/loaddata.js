const maxWidth = 1500, maxHeight = 1500, maxTop = 50;
let totalData;
function loadBlogPostData(draw, top){
    ...
    d3.tsv(fileName, function(error, rawData) {
        ...
        processSudden(data);
        totalData = getTop(data, topics, maxTop).slice(1);
        data = getTop(data, topics, top).slice(1);   // omit first timestep
...
function loadAuthorData(draw, top){
    ...
    d3.tsv(fileName, function(error, rawData) {
        ...
        processSudden(data);
        data = getTop(data, topics, top).slice(1);
    ...
    data.forEach((month,i) => {
        ...
        topics.forEach(topic => {
            words[topic].forEach((d) => {
                ...
                let tf = d["frequency"]/docFreq[i];
                let idf = Math.log10(N/df);
                d.tf_idf = tf*idf;
            })
        })
    });
